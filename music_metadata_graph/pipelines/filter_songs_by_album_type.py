...
def fetch_removed_rows(connection: sqlite3.Connection) -> list[dict[str, Any]]:
    ...
    rows = connection.execute(
        ...
        SELECT
            ...
            'album_type_not_allowed_duplicate_name' AS reason_code
        FROM songs
    ...
    return prepare_song_csv_rows(
        ...
        progress_label="prepare_album_type_rejection_csv",
    )


def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    write_song_csv(path, rows, include_credits=False, include_reason=True)
...
def run(config: FilterConfig) -> dict[str, Any]:
    ...
    try:
        ...
        print(
            json.dumps(
                {"stage": "fetch_album_type_removed_rows", "songs_before": before_count},
                ensure_ascii=False,
            ),
            flush=True,
        )
        ...
        print(
            json.dumps(
                {
                    "stage": "write_album_type_rejection_csv",
        ...
        print(
            json.dumps(
                {"stage": "delete_album_type_removed_songs", "rows": len(removed_rows)},
                ensure_ascii=False,
            ),
            flush=True,
        )
        with connection:
            connection.executemany(
                "DELETE FROM songs WHERE mid = ?", [(row["song_mid"],) for row in removed_rows]
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Step 10: keep only Single, EP, and 录音室专辑 songs."
    )
...
def _main() -> None:
    ...
    print(
        json.dumps(
            run(FilterConfig(db_path=args.db, rejection_csv=args.rejection_csv)),
            ensure_ascii=False,
            indent=2,
        )
    )
