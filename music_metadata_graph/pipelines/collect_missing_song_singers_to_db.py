...
def load_existing_song_tab_targets(connection: sqlite3.Connection, config: CollectConfig) -> tuple[SingerTarget, ...]:

    source_root = config.raw_dir / "singer_homepage_song_tab"

    if not source_root.exists():

        raise FileNotFoundError(f"Singer homepage song-tab raw directory does not exist: {source_root}")
    ...
    targets = [SingerTarget(mid=str(row["mid"]), name=str(row["name"])) for row in rows if str(row["mid"]) in available_mids]

    if not targets:

        raise ValueError("No current database artists have existing singer homepage song-tab raw JSON.")
        ...
        for path in files:
            ...
            for song in ((payload.get("SongTab") or {}).get("List") or []):
                ...
                for singer in song.get("singer") or []:
                    ...
                    item = missing.setdefault(singer_mid, {"song_count": 0, "examples": [], "names": []})
    ...
    for start in range(0, len(pending), config.batch_size):
        ...
        try:
            ...
            for index, singer, path, request in batch:
                ...
                try:
                    ...
                    failures.append(FetchFailure(mid=singer.mid, path=path, reason=f"{type(item_exc).__name__}: {item_exc}"))
        ...
        for (index, singer, path, _request), result in zip(batch, fetched_results, strict=True):
            ...
            if isinstance(result, Exception):
                ...
                failures.append(FetchFailure(mid=singer.mid, path=path, reason=f"{type(result).__name__}: {result}"))
...
def extract_singer_row(
    ...
    if not icon:
        ...
        icon = str(base_info.get("background_image") or base_info.get("BackgroundImage") or "").strip()
    ...
    return (

        {
            ...
            "other_name": str(singer.get("other_name") or singer.get("transName") or singer.get("ForeignName") or ""),
...
async def collect(config: CollectConfig) -> None:
    ...
    missing_singers, song_rows, singer_items_without_mid = load_missing_song_singers(config, singer_targets)
    ...
    try:
        ...
        for loaded_index, singer, payload, status, path in loaded:
            ...
            if row is None:
                ...
                print(f"[{display_index}/{len(missing_singers)}] singer_mid={singer.mid} status={status} skipped={reason} saved={path.as_posix()}")
        ...
        for failure in fetch_failures:
            ...
            print(f"singer_mid={failure.mid} status=failed reason={failure.reason} saved={failure.path.as_posix() if str(failure.path) else ''}")
    ...
    if fetch_failures:
        ...
        raise RuntimeError(f"Failed to fetch {len(fetch_failures)} singer info request(s). Rerun the command to continue from cached successes. Failed mids: {failed_mids}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Collect missing singer info from song singer mids and import it into SQLite.")
    ...
    parser.add_argument("--all", action="store_true", dest="all_available_song_tabs", help="Scan song-tab raw only for singers selected by the current step-2 singer-list import rules.")
    ...
    parser.add_argument("--max-singers", type=int, default=None, help="Limit missing singer requests for smoke tests.")
