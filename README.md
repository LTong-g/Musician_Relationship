...
```powershell
python -m music_metadata_graph.pipelines.build_avatar_atlas
```

该命令默认使用 `auto` 模式：当前输入和现有图集指纹一致时跳过重建；需要强制重建时使用 `--force`；只想验证发布图集是否匹配当前输入时使用 `--check`。
重新生成标准图谱网页：
...
- `data/raw/`：接口响应缓存和最小化证据缓存，用作可追溯证据；歌词补充只长期保存头部解析证据，不保存完整歌词。
- `data/raw/qqmusic/avatar_cache/`：原始头像下载缓存。
...
- `site_assets/avatar_atlas_150/`：共享的 150x150 WebP 头像图集。
- `logs/runs/`：每次正式脚本运行的日志。

原始 raw、数据库、validation、日志和完整头像缓存默认不应提交到 Git。共享头像图集属于发布资源，体积验收后可随 GitHub Pages 站点提交。
