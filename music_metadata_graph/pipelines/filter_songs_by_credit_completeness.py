...
def incomplete_credit_reason_code(row: dict[str, Any]) -> str:
    missing_lyricist = int(row.get("lyricist_count") or 0) == 0

    missing_composer = int(row.get("composer_count") or 0) == 0

    if missing_lyricist and missing_composer:
        return "missing_lyricist_and_composer"

    if missing_lyricist:
        return "missing_lyricist"
...
def run(config: FilterConfig) -> dict[str, Any]:
    with connect(config.db_path) as connection:
        ...
        print(
            json.dumps(
                {"stage": "fetch_incomplete_credit_rows", "songs_before": before_count},
                ensure_ascii=False,
            ),
            flush=True,
        )
        ...
        removed_rows = [
            {**row, "reason_code": incomplete_credit_reason_code(row)} for row in removed_rows
        ]
        ...
        print(
            json.dumps(
                {
                    "stage": "write_incomplete_credit_rejection_csv",
        ...
        write_rejection_csv(
            ...
            prepare_csv_rows(connection, removed_rows, "prepare_incomplete_credit_rejection_csv"),
        )
        ...
        if config.temp_kept_csv is not None or config.four_singer_temp_kept_csv is not None:
            ...
            if config.temp_kept_csv is not None:
                print(
                    json.dumps(
                        {
                            "stage": "write_complete_credit_temp_csv",
                            "rows": len(all_kept_rows),
                ...
                write_csv(
                    ...
                    prepare_csv_rows(connection, all_kept_rows, "prepare_complete_credit_temp_csv"),
                )

                exported_temp_kept_rows = len(all_kept_rows)
            ...
            if config.four_singer_temp_kept_csv is not None:
                ...
                four_singer_kept_rows = [
                    row for row in all_kept_rows if str(row["song_mid"]) in temp_export_song_mids
                ]

                print(
                    json.dumps(
                        {
                            "stage": "write_complete_credit_four_singer_temp_csv",
                ...
                write_csv(
                    ...
                    prepare_csv_rows(
                        connection,
                        four_singer_kept_rows,
                        "prepare_complete_credit_four_singer_temp_csv",
                    ),
                )
        ...
        with connection:
            print(
                json.dumps(
                    {"stage": "delete_incomplete_credit_songs", "rows": len(removed_mids)},
                    ensure_ascii=False,
                ),
                flush=True,
            )
    ...
    return {
        ...
        "songs_after_step14": after_count,
        "song_singer_rows_after_step14": song_singer_rows,
        "song_credit_rows_after_step14": song_credit_rows,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Step 14: remove songs without both lyricist and composer credits."
    )
