...
async def collect_combination(client: Client, config: CollectConfig, combination: FilterCombination) -> dict[str, Any]:
...
async def collect(config: CollectConfig) -> None:
    ...
    print(json.dumps({"combinations": len(targets), "page_size": config.page_size}, ensure_ascii=False))
    ...
    try:
        ...
        for index, combination in enumerate(targets, 1):
            ...
            print(f"[{index}/{len(targets)}] collect singer list: {combination_dir_name(combination)}")
    ...
    for raw_value in parsed_values:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Invalid {enum_type.__name__} value: {raw_value}. Choices: {choices}") from exc
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Collect raw QQ Music singer list index JSON pages only.")

    parser.add_argument("--raw-dir", type=Path, default=DEFAULT_RAW_DIR)

    parser.add_argument("--page-size", type=int, default=DEFAULT_PAGE_SIZE)

    parser.add_argument("--max-pages", type=int, default=None, help="Limit pages for smoke tests. Omit to request the full singer list.")
    parser.add_argument("--mvp", action="store_true", help="MVP mode: use shared raw cache but only ensure the first page exists.")
    ...
    parser.add_argument("--sex", action="append", help="SexType list. Use names or values, repeated or comma-separated. Defaults to ALL.")

    parser.add_argument("--genre", action="append", help="GenreType list. Use names or values, repeated or comma-separated. Defaults to ALL.")

    parser.add_argument("--index", action="append", help="IndexType list. Use names or values, repeated or comma-separated. Defaults to ALL.")
