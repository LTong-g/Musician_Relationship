...
async def execute_request_batch(
    ...
    batch: list[tuple[int, str, Path, Any]],
    ...
    for (index, song_mid, path, _request), result in zip(batch, fetched_results, strict=True):
        if isinstance(result, Exception):
            failures.append(
                FetchFailure(
                    song_mid=song_mid, path=path, reason=f"{type(result).__name__}: {result}"
                )
            )
            ...
            missing.append(
                {
                    ...
                    "producer_type": (
                        producer.get("type")
                        if producer.get("type") is not None
                        else producer.get("Type") or ""
                    ),
...
async def collect(config: ProducerConfig) -> None:
    ...
    pending_requests: list[tuple[int, str, Path, Any]] = []

    async def handle_loaded(loaded_rows: list[tuple[int, str, dict[str, Any], str, Path]]) -> None:
        nonlocal fetched, cache_hits

        for loaded_index, song_mid, payload, status, path in loaded_rows:
            ...
            if (
                status == "fetched"
                or display_index == len(song_mids)
                or display_index % PROGRESS_EVERY == 0
            ):
                print(
                    f"[{display_index}/{len(song_mids)}] song_mid={song_mid} status={status} saved={path.as_posix()}",
                    flush=True,
                )

    async def flush_pending_requests() -> None:
        if not pending_requests:
            return

        batch = list(pending_requests)

        pending_requests.clear()

        loaded, batch_failures = await execute_request_batch(client, config, batch)

        fetch_failures.extend(batch_failures)
        ...
        for index, song_mid in enumerate(song_mids):
            ...
            if path.exists() and not config.force:
                ...
                if payload is not None:
                    await handle_loaded([(index, song_mid, payload, "cache_hit", path)])

                    continue

            request = dataclass_replace(client.song.get_producer(song_mid), response_model=None)

            pending_requests.append((index, song_mid, path, request))

            if len(pending_requests) >= config.batch_size:
                await flush_pending_requests()
    ...
    if fetch_failures:
        ...
        raise RuntimeError(
            f"Failed to fetch {len(fetch_failures)} song producer request(s). Rerun the command to continue from cached successes. Failed song mids: {failed_mids}"
        )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Step 11: collect QQ Music song producer raw JSON and inspect lyricist/composer missing mids."
    )
    ...
    parser.add_argument(
        "--force", action="store_true", help="Refetch and overwrite cached producer raw JSON."
    )
