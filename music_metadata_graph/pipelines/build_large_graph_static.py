...
function renderGraph() {
  ...
  $("graph-note").textContent = `${currentScopeLabel()} · ${formatNumber(graph.nodes.length)} 个节点，${formatNumber(graph.edges.length)} 条边 · 绘图区配置按 force-graph 官方 large-graph 示例，保留鼠标交互但禁用节点拖动`;
}
"""
...
def large_graph_html_document_from_assets(title: str) -> str:
    return html_document(
        title,
...
def write_visualization(config: BuildConfig) -> dict[str, Any]:
    ...
    return {
        ...
        "drawing": "Standard shell with force-graph example/large-graph drawing configuration, pointer interaction enabled, and node dragging disabled",
    }
