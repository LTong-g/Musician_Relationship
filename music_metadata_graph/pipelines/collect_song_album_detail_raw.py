...
DEFAULT_FAILURE_JSON = Path("data/processed/validation/album_detail_fetch_failures/album_detail_fetch_failures.json")
DEFAULT_MVP_FAILURE_JSON = Path("data/processed/validation_mvp/album_detail_fetch_failures/album_detail_fetch_failures.json")
...
def load_album_targets(config: CollectConfig, singer_targets: tuple[SingerTarget, ...]) -> tuple[list[AlbumTarget], int, int]:
    ...
    for start in range(0, len(pending), config.batch_size):
        ...
        try:
            ...
            for index, album, path, request in batch:
                ...
                try:
                    ...
                    failures.append(FetchFailure(key=album.key, path=path, reason=f"{type(item_exc).__name__}: {item_exc}"))
        ...
        for (index, album, path, _request), result in zip(batch, fetched_results, strict=True):
            ...
            if isinstance(result, Exception):
                ...
                failures.append(FetchFailure(key=album.key, path=path, reason=f"{type(result).__name__}: {result}"))
...
async def collect(config: CollectConfig) -> None:
    ...
    try:
        ...
        for loaded_index, album, payload, status, path in loaded:
            ...
            album_obj = payload.get("album") or payload.get("basicInfo") or {}

            album_name = album_obj.get("name") or album_obj.get("albumName") or album.key

            print(

                f"[{display_index}/{len(albums)}] album={album_name} key={album.key} "
        ...
        for failure in fetch_failures:
            ...
            print(f"album_key={failure.key} status=failed reason={failure.reason} saved={failure.path.as_posix()}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Collect raw QQ Music album detail JSON from singer homepage song-tab raw JSON.")
    ...
    parser.add_argument("--name", action="append", help="Singer exact name whose song-tab raw JSON should be used. Can be repeated or comma-separated.")

    parser.add_argument("--max-albums", type=int, default=None, help="Limit albums for smoke tests.")
...
def _main() -> None:
    ...
    failure_json = args.failure_json if args.failure_json is not None else (DEFAULT_MVP_FAILURE_JSON if args.mvp else DEFAULT_FAILURE_JSON)
