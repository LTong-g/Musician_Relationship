...
DEFAULT_REJECTION_CSV = Path("data/processed/validation/album_import_rejections/csv_views/album_import_rejections.csv")
...
def split_valid_and_rejected_albums(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
...
def build_rejection_row(row: dict[str, Any], reasons: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason_flags": ";".join(sorted(set(reasons))),
...
def write_rejection_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    with path.open("w", encoding="utf-8-sig", newline="") as file:
        ...
        writer = csv.DictWriter(file, fieldnames=ALBUM_REJECTION_CSV_FIELDS, extrasaction="ignore", lineterminator="\r\n")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Import raw QQ Music album detail JSON into SQLite.")
...
def _main() -> None:
    ...
    result = run(ImportConfig(raw_dir=args.raw_dir, db_path=args.db, rejection_csv=args.rejection_csv))
