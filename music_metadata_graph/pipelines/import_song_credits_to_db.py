...
DEFAULT_TEMP_SONGS_CSV = Path("data/processed/validation/temp_song_filtering/csv_views/songs_after_step10_credit_import.csv")
CREDIT_TITLES = ("作词", "作曲")
...
def load_unique_artist_name_map(connection: sqlite3.Connection) -> tuple[dict[str, dict[str, Any]], int]:
    ...
    ambiguous_names = sum(1 for rows in rows_by_name.values() if len({str(row["mid"]) for row in rows}) > 1)
    return unique_map, ambiguous_names


def resolve_missing_artist_name_rows(name: str, artist_name_map: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
...
def load_credit_rows(connection: sqlite3.Connection, config: ImportConfig) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        for group in producer_groups(payload):
            ...
            for order, producer in enumerate(producers, 1):
                ...
                for artist in resolved_artists:
                    ...
                    next_order_by_song_role[order_key] = next_order_by_song_role.get(order_key, 0) + 1
                    credit_rows.append(
                        {
                            "song_mid": song_mid,
                            "role": role,
...
def fetch_song_csv_rows(connection: sqlite3.Connection, config: ImportConfig) -> list[dict[str, Any]]:
    ...
    if config.artist_mid:
    ...
    elif config.temp_export_artist_names:

        placeholders = ", ".join("?" for _ in config.temp_export_artist_names)
        ...
        params.extend(config.temp_export_artist_names)
...
def run(config: ImportConfig) -> dict[str, Any]:
    ...
    with connect(config.db_path) as connection:
        ...
        if config.temp_songs_csv is not None:
            ...
            temp_rows = prepare_song_csv_rows(connection, fetch_song_csv_rows(connection, config), include_credits=True)
    ...
    return {
        ...
        "temp_songs_csv": config.temp_songs_csv.as_posix() if config.temp_songs_csv is not None else "",

        "temp_export_artist_names": list(config.temp_export_artist_names) if not config.artist_mid and not config.artist_name else [],
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Import lyricist/composer credits from QQ Music producer raw JSON.")
    ...
    parser.add_argument("--no-temp-songs-csv", action="store_true", help="Do not write the temporary song CSV after importing credits.")

    parser.add_argument("--artist-mid", default=None, help="Only import credits for songs whose singer list contains this artist mid.")

    parser.add_argument("--artist-name", default=None, help="Only import credits for songs whose singer list contains this exact artist name.")
    ...
    parser.add_argument("--all-temp-songs-csv", action="store_true", help="Export all current songs to the temporary song CSV.")
